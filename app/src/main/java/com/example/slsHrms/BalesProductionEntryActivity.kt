...
class BalesProductionEntryActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_bales_production_entry)
            ...
            .enqueue(object : Callback<SpellResponse> {
                override fun onResponse(call: Call<SpellResponse>, response: Response<SpellResponse>) {
                    ...
                    spEntrySpell.adapter = ArrayAdapter(
                        this@BalesProductionEntryActivity,
        ...
        RetrofitClient.getApiService(this).getFinishingQualities()
            .enqueue(object : Callback<FinishingQualityResponse> {
                ...
                override fun onFailure(call: Call<FinishingQualityResponse>, t: Throwable) {
                    Toast.makeText(this@BalesProductionEntryActivity,
    ...
    private fun updateStdWeightInfo() {
        ...
        if (q != null && std != null) {
            ...
            tvStdWeight.text = "Std Weight / Bale — ${q.label()}: ${fmt(std)}"
        } else {
    ...
    private fun saveEntry() {
        ...
        if (editId != null) {
            val updReq = BalesProdUpdateRequest(
                fngQualityId  = quality.qualityId,
                customerId    = customer.customerId,
                prodBales     = bales,
                actBaleWeight = weight
            )
            RetrofitClient.getApiService(this).updateBalesProdEntry(editId, updReq)
                .enqueue(object : Callback<BalesProdSaveResponse> {
                    override fun onResponse(call: Call<BalesProdSaveResponse>, response: Response<BalesProdSaveResponse>) {
                        ...
                        Toast.makeText(this@BalesProductionEntryActivity, msg, Toast.LENGTH_SHORT).show()
        ...
        RetrofitClient.getApiService(this).saveBalesProdEntry(req)
            .enqueue(object : Callback<BalesProdSaveResponse> {
                override fun onResponse(call: Call<BalesProdSaveResponse>, response: Response<BalesProdSaveResponse>) {
                    btnSave.isEnabled = true
    ...
    private fun beginEdit(e: BalesProdEntry) {
        ...
        etNoOfBales.setText(e.prodBales?.toString() ?: "")
    ...
    private fun confirmDelete(e: BalesProdEntry) {
        ...
        android.app.AlertDialog.Builder(this)
            ...
            .setMessage("${e.quality ?: "-"} | ${e.customer ?: "-"} | ${e.prodBales ?: 0} bales")
            .setPositiveButton("Delete") { _, _ ->
                RetrofitClient.getApiService(this).deleteBalesProdEntry(id)
                    .enqueue(object : Callback<BalesProdSaveResponse> {
                        override fun onResponse(call: Call<BalesProdSaveResponse>, response: Response<BalesProdSaveResponse>) {
                            Toast.makeText(this@BalesProductionEntryActivity,
    ...
    private fun loadEntries() {
        ...
        RetrofitClient.getApiService(this).getBalesProdEntries(
            date    = entryDate,
            spellId = spellId
    ...
    inner class EntriesAdapter : RecyclerView.Adapter<EntriesAdapter.VH>() {
        ...
        inner class VH(v: View) : RecyclerView.ViewHolder(v) {
            ...
            fun bind(e: BalesProdEntry) {
                ...
                tvBales.text    = (e.prodBales ?: 0).toString()
